...
import { Mail, Save, User as UserIcon, Camera, ArrowLeft } from 'lucide-react'
...
export function ProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation chặn người dùng chỉ nhập toàn dấu cách
    if (!formData.fname.trim() || !formData.lname.trim()) {
      alert('Họ và tên không được để trống hoặc chỉ chứa khoảng trắng!')
      return
    }
    ...
    try {
      ...
      // SỬA ĐỔI: Dùng unknown thay cho any
      console.error('Lỗi cập nhật profile:', error)

      let errorMsg = 'Có lỗi xảy ra khi cập nhật hồ sơ'

      // SỬA ĐỔI: Type narrowing (kiểm tra kiểu an toàn) với isAxiosError
        ...
        <div className='bg-white rounded-xl shadow-sm border border-gray-100 p-6'>
          ...
          <div className='mb-8 border-b border-gray-200 pb-6 flex items-center space-x-5'>
            ...
            <div>
              ...
              <p className='text-sm text-gray-500 mt-1'>Quản lý thông tin cá nhân của bạn trên SCORMGO</p>
            </div>
          </div>
